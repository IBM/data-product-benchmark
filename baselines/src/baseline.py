...
def ingest_collection(client, data, collection_name = "text_collection", emb_dim=768, remove_if_exists=False):
    ...
    if client.has_collection(collection_name=collection_name):
        if remove_if_exists:
            ...
            print(f"Collection {collection_name} already exists — skipping creation and insert.")
            return
...
def prepare_data(dataset_name, embedding_model, device):

    corpus_dataset_id = f"{dataset_name}-corpus"
    repo_id = "ibm-research/data-product-benchmark"
    print(f"loading HF dataset {corpus_dataset_id} from {repo_id}")
    dataset = load_dataset(repo_id, corpus_dataset_id)
...
def prepare_queries(dataset_name, split, embedding_model, device):

    repo_id = "ibm-research/data-product-benchmark"
    print(f"loading HF dataset {dataset_name}-{split} from {repo_id}")
    dataset = load_dataset(repo_id, dataset_name, split=split)

    ids = [dpr["dpr_id"] for dpr in dataset]
    queries = [dpr["DPR"] for dpr in dataset]
...
def main(args):
    ...
    if client.has_collection(collection_name="text_collection") and client.has_collection(collection_name="table_collection"):
        ...
        table_rows, text_rows = prepare_data(dataset_name, embedding_model, device)
    ...
    query_rows = prepare_queries(dataset_name, split, embedding_model, device)
    ...
    result_summary = {
        ...
        "bm25_embedding_overlap": {"text": total_overlap_text/total_rrf, "table": total_overlap_table/total_rrf}
    }
...
if __name__ == "__main__":
    ...
    p.add_argument("--db_path",          type=str, default="TATQA.db")
